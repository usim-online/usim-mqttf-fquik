...
//    override val plusBannerIntent by lazy { plusBanner.clicks() }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Disable drawer items not applicable to fengni text-only mode
        listOf(
            binding.drawer.backup to true,      // QUIK+ feature
            binding.drawer.scheduled to true,   // QUIK+ feature
            binding.drawer.blocking to true,    // blocking
            binding.drawer.messageUtils to true, // message utils
            binding.drawer.invite to true,       // invite friends
        ).forEach { (view, disable) ->
            if (disable) {
                view.isEnabled = false
                view.alpha = 0.4f
            }
        }
        // Hide QUIK+ upsell banner and rating prompt
        binding.drawer.plusBanner.isVisible = false
        binding.drawer.rateLayout.isVisible = false
    ...
    private val MIN_SYNC_DISPLAY_MS = 500L
...
//        plus.isVisible = state.upgraded
        ...
        when (state.syncing) {
            is SyncRepository.SyncProgress.Idle -> {
                val elapsed = System.currentTimeMillis() - syncStartedAt
                if (elapsed < MIN_SYNC_DISPLAY_MS) {
                    syncingBinding.root.postDelayed({
                        syncingBinding.root.isVisible = false
                        snackbarBinding.root.isVisible = (!state.fengniConfigured ||
                                !state.smsPermission ||
        ...
        when {
            !state.fengniConfigured -> {
                snackbarBinding.snackbarTitle.setText(R.string.main_fengni_config_title)
                snackbarBinding.snackbarMessage.setText(R.string.main_fengni_config_message)
                snackbarBinding.snackbarButton.setText(R.string.main_fengni_config_import)
            }
        ...
        // Online count — shown when fengni is configured
        if (state.fengniConfigured && state.onlineCount > 0) {
            binding.onlineCount.text = "${state.onlineCount} 在线"
            binding.onlineCount.setVisible(true)
        } else {
            binding.onlineCount.setVisible(false)
        }
    }
