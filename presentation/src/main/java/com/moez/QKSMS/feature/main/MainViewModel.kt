...
import com.moez.QKSMS.fengni.FengniConfigManager
import com.moez.QKSMS.fengni.FengniMqttService
...
class MainViewModel @Inject constructor(
    ...
    private val configManager: FengniConfigManager,
    ...
    override fun bindView(view: MainView) {
        ...
        // Immediately sync current permission state (don't wait for onResume)
        newState {
            copy(
                fengniConfigured = configManager.isConfigured(),
                smsPermission = permissionManager.hasReadSms(),
                contactPermission = permissionManager.hasContacts(),
                notificationPermission = permissionManager.hasNotifications()
            )
        }

        // Auto-start fengni MQTT service if config is already imported.
        // This ensures the service recovers after process restart or app reopen.
        if (configManager.isConfigured()) {
            context.startService(Intent(context, FengniMqttService::class.java))
        }

        // Listen for online count updates from FengniMqttService
        val onlineCountReceiver = object : BroadcastReceiver() {
            override fun onReceive(ctx: Context?, intent: Intent?) {
                val count = intent?.getIntExtra(FengniMqttService.EXTRA_ONLINE_COUNT, 0) ?: 0
                newState { copy(onlineCount = count) }
            }
        }
        LocalBroadcastManager.getInstance(context).registerReceiver(
            onlineCountReceiver,
            IntentFilter(FengniMqttService.ACTION_ONLINE_COUNT)
        )
        ...
        // If fengni config state changes on resume, reflect it
        view.activityResumedIntent
            .filter { resumed -> resumed }
            .observeOn(Schedulers.io())
            .map { configManager.isConfigured() }
            .distinctUntilChanged()
            .doOnNext { configured -> newState { copy(fengniConfigured = configured) } }
            .autoDisposable(view.scope())
            .subscribe()

        // On resume, query the service for current online count
        view.activityResumedIntent
            .filter { resumed -> resumed }
            .observeOn(Schedulers.io())
            .filter { configManager.isConfigured() }
            .autoDisposable(view.scope())
            .subscribe {
                context.startService(
                    Intent(context, FengniMqttService::class.java)
                        .setAction(FengniMqttService.ACTION_ONLINE_COUNT_QUERY)
                )
            }
        ...
        view.activityResumedIntent
            .filter { resumed -> resumed }
            .observeOn(Schedulers.io())
            .map { permissionManager.hasReadSms() && permissionManager.hasContacts() }
            .distinctUntilChanged()
            .skip(1)
...
//                }
        ...
        view.snackbarButtonIntent
                .withLatestFrom(state) { _, state ->
                    when {
                        !state.fengniConfigured -> navigator.showFengniConfigImport()
