...
class ConversationRepositoryImpl @Inject constructor(
    ...
    override fun createConversation(addresses: Collection<String>, sendAsGroup: Boolean) =
        TelephonyCompat.getOrCreateThreadId(context, addresses.toSet())
            ...
            ?: run {
                // Fallback: ContentProvider has no thread for these addresses
                // (e.g. fengni MQTT messages bypass the system SMS database).
                // Generate a deterministic threadId from the normalized address
                // — must match the threadId computed by syncProviderMessage().
                val generatedThreadId = addresses.firstOrNull()
                    ?.let { phoneNumberUtils.normalizeNumber(it) }
                    ?.hashCode()
                    ?.toLong() ?: 0L
                if (generatedThreadId != 0L)
                    createEmptyConversation(generatedThreadId, addresses, sendAsGroup)
                else null
            }
    ...
    override fun deleteConversations(vararg threadIds: Long) {
        ...
        threadIds.forEach {
            try {
                ...
                // Not the default SMS app; ContentProvider thread cleanup not permitted.
                // Realm data was already deleted above — this is best-effort only.
            }
        }
    }
    ...
    private fun createEmptyConversation(threadId: Long, addresses: Collection<String>, sendAsGroup: Boolean): Conversation {
        Realm.getDefaultInstance().use { realm ->
            ...
            // Populate lastMessage so the conversation passes the inbox query's
            // isNotNull("lastMessage") filter immediately — before the async
            // updateConversations() call in SendNewMessage completes.
            val lastMsg = realm.where(Message::class.java)
                .sort("date", Sort.DESCENDING)
                .equalTo("threadId", threadId)
                .findFirst()
            if (lastMsg != null) {
            ...
            realm.executeTransaction { realm.copyToRealmOrUpdate(conversation) }
            return conversation
        }
    }
