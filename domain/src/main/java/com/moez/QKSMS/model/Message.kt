...
open class Message : RealmObject() {
    companion object {
        ...
        /** Maximum entries in fengniUris before eviction kicks in */
        private const val MAX_FENGNI_URIS = 5000

        /** Maps message IDs to fengni URIs for send flow bypassing ContentProvider */
        val fengniUris = java.util.concurrent.ConcurrentHashMap<Long, Uri>()

        /**
         * Remove a fengni URI mapping when a message is deleted or the send
         * pipeline completes. Callers should invoke this after the Realm
         * Message is removed.
         */
        fun removeFengniUri(id: Long) {
            fengniUris.remove(id)
        }

        /**
         * Evict oldest entries when the map exceeds [MAX_FENGNI_URIS].
         * Realm IDs are monotonic, so entries with the smallest keys are the oldest.
         */
        fun trimFengniUris() {
            if (fengniUris.size > MAX_FENGNI_URIS) {
                // Keep the most recent ~80%; evict the oldest ~20% by key
                val keys = fengniUris.keys().toList().sorted()
                val cutoffIndex = (keys.size * 0.2).toInt().coerceAtLeast(1)
                val threshold = keys[cutoffIndex]
                val iter = fengniUris.entries.iterator()
                while (iter.hasNext()) {
                    if (iter.next().key < threshold) iter.remove()
                }
            }
        }
    }
